...
Rcpp::List  calc_p_mat(const double tau_k, const double phi_k,
                       arma::mat Y_mat_slice, int nrint_k, int ntint_k,
  ...
  arma::mat Y_mat = Y_mat_slice(arma::span(0, nrint_k-1), arma::span(0, ntint_k-1));
  Rcpp::NumericMatrix Y = Rcpp::wrap(Y_mat);
  ...
  Rcpp::NumericMatrix tmp1(clone(CDF_binned));
  if (tmp1.nrow()>1){
  ...
  Rcpp::NumericMatrix p_matrix(clone(tmp1));
  if (p_matrix.ncol()>1){
...
double nll_fun(Rcpp::NumericVector params, arma::mat X1, arma::mat X2,
  ...
  for(int k = 0; k < nsurvey; ++k){
        ...
        Rcpp::List pmat_out = calc_p_mat(tau(k), 
          ...
          Rcpp::NumericMatrix Y = pmat_out["Y"];
